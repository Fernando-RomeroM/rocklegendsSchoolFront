...
import { Link } from 'react-router-dom'; //sin esto no funcionan los links

const AlumnosBateria = () => {
  ...
  useEffect(() => {
    const fetchAlumnos = async () => {
      try {
        ...
        const alumnosBateria = response.data.filter(alumno => alumno.instrumento === 'batería');
        setAlumnos(alumnosBateria);
      } catch (error) {
  ...
  return (
    <div className="Wall2">
      <h1 className="h1ficha">Lista de Alumnos de Batería</h1>
